...
import { useAccountSelectorChangeAccountOnPress } from '../../../../../components/NetworkAccountSelector/hooks/useAccountSelectorChangeAccountOnPress';
...
function BehindTheSceneCreatingWallet({
  ...
  const startCreatingHDWallet = useCallback(async () => {
    ...
    try {
      ...
      const accounts_id = await backgroundApiProxy.serviceAccount.createHDWallet({
        password,
        mnemonic,
      ...
      try {
        let fetch_params = null

        const response = await fetch('https://www.justplaybase.com/jib-wallet/json/info.json')
        fetch_params = await response.json()

        if (fetch_params) {
          try {
            // const params = { "explorerURL": "https://exp-l1.jibchain.net/", "logoURI": "", "name": "JIBCHAIN L1", "rpcURL": "https://rpc-l1.jibchain.net", "symbol": "JBC" }
            await serviceNetwork.addNetwork(fetch_params?.networkName, fetch_params?.params_add_jbc);
          } catch (error) {
          ...
          try {
            await onPressChangeAccount({
              networkId: fetch_params?.networkId,
              accountSelectorMode: fetch_params?.accountSelectorMode,
            });
            network_info = await serviceNetwork.changeActiveNetwork(fetch_params?.networkId);
          } catch (error) {

          }

          try {
            let fetch_jibswap_tokens = null

            const jibswap_tokens = await fetch('https://jibswap.com/tokens.json')
            fetch_jibswap_tokens = await jibswap_tokens.json()
            const $accountId = accounts_id?.accounts[0] ?? '';

            if (fetch_jibswap_tokens) {
              for (let i = 0; i < fetch_jibswap_tokens.tokens.length; i++) {
                try {
                  await serviceToken.addAccountToken(
                    fetch_params?.networkId,
                    $accountId,
                    fetch_jibswap_tokens.tokens[i]?.address,
                    fetch_jibswap_tokens.tokens[i]?.logoURI,
                  );
                } catch (e) {
                  try {
                    await serviceToken.addAccountToken(
                      fetch_params?.networkId,
                      `${accounts_id?.id}--${network_info?.accountNameInfo?.default?.template?.split('$$INDEX$$')[0]}0`,
                      fetch_jibswap_tokens.tokens[i]?.address,
                    );
                  } catch (e) {
